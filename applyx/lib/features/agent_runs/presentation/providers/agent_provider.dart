...
class AgentRunNotifier extends AsyncNotifier<AgentRun?> {
  ...
  void simulateProgress() {
    ...
    final runningIndex = steps.indexWhere((s) => s.status == AgentStatus.running);
    if (runningIndex == -1) return;

    steps[runningIndex] = steps[runningIndex].copyWith(status: AgentStatus.completed);
    
    if (runningIndex + 1 < steps.length) {
      steps[runningIndex + 1] = steps[runningIndex + 1].copyWith(status: AgentStatus.running);
    }
